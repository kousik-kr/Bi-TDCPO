...
public class BidirectionalLabeling implements Callable<List<Label>>{
	private int destination;
	private Label topLabel;
	private double budget;
	private boolean master = false;
	
	public BidirectionalLabeling(int destination, double b, Label label){
	...
	@Override
	public List<Label> call() throws InterruptedException, ExecutionException{
...
//		}
		...
		List<BidirectionalLabeling> labelQueue = new ArrayList<BidirectionalLabeling>();
...
//			}
			if(Graph.get_node(j).isFeasible() && !topLabel.getVisited(j)) {
				...
				if(arrivalTime!=null) {
					...
					if(j==destination) {
						destinationLabels.add(newLabel);
						
					}
					else {
						BidirectionalLabeling newthread = new BidirectionalLabeling(destination, budget, newLabel);
		...
		if(labelQueue.size()>0) {
			...
			//for(Labeling labeling : labelQueue) {
				...
				if(labels != null && labels.size()>0) {
					destinationLabels.addAll(labels);
				}
			}	
			if(master) {
				if(!BidirectionalAstar.isMemoryUpdated()) 
