...
public class BackwardLabeling implements Callable<List<Label>>{
	private int destination;
	private Label topLabel;
	private double budget;
	private boolean master = false;
	
	public BackwardLabeling(int destination, double b, Label label){
	...
	@Override
	public Map<Integer, List<Label>> call() throws InterruptedException, ExecutionException{

		Map<Integer,Label> jointLabels = new HashMap<Integer,Label>();
...
//		}
		...
		List<BackwardLabeling> labelQueue = new ArrayList<BackwardLabeling>();
...
//			}
			if(Graph.get_node(j).isFeasible() && !topLabel.getVisited(j)) {
				...
				while(current_arrivaltime_function!=null) {
					...
					for(int time_point=0;time_point<current_arrivaltime_function.getBreakpoints().size();time_point++) {
						...
						//to reach j to d
						double min_required_budget = Graph.get_node(j).euclidean_distance(Graph.get_node(destination))/BidirectionalAstar.MAX_SPEED;
						...
						if((Graph.get_node(j).get_backward_hScore() >= new_arrival_time)) {
							if((new_arrival_time + min_required_budget - arrival_time_breakpoint.getX())<=budget)	{
								...
								if(!previous_status){//outside to inside....starting new function
									...
									if(!is_first) {
										...
										//TODO verify all
										double allotted_budget = budget - min_required_budget; 
				...
				if(arrivalTime!=null) {
					...
					if(j==destination) {
						jointLabels.add(newLabel);
						
					}
					else {
						BackwardLabeling newthread = new BackwardLabeling(destination, budget, newLabel);
						labelQueue.add(newthread);
						//newthread.fork();
					}
				}
			}
		...
		if(labelQueue.size()>0) {
			List<Future<List<Label>>> future = BidirectionalAstar.pool.invokeAll(labelQueue);
			for(Future<List<Label>> x : future) {
			//for(Labeling labeling : labelQueue) {
				//while(!x.isDone()) continue;
				//labeling.join();//x.get();
				List<Label> labels = x.get();//labeling.call();//
				if(labels != null && labels.size()>0) {
					jointLabels.addAll(labels);
				}
			}	
			if(master) {
				if(!BidirectionalAstar.isMemoryUpdated()) 
